...
from circuits_FLAP import choose_hybrid_FLAP_circuit, choose_contrastive_FLAP_circuit, choose_vanilla_FLAP_circuit
from dataset.loader import load_dataset
from utils.config import set_PATH

def FLAP_patching(args,
        ks, 
        scales,
    ...
    for k in ks:
        for s in scales:
            for min_acts in min_activation_thresholds:
                ...
                if args.mode=="sqrt":
                        result_folder = f"{args.model_name}/{args.task}/APP/min_threshold-{args.min_activation_threshold}/scale-{args.scale}"
                else:
                        result_folder = f"{args.model_name}/{args.task}/APP/min_threshold-{args.min_activation_threshold}/alpha_{args.alpha}-scale_{args.scale}"
...
if __name__ == "__main__":
    ...
    min_activation_thresholds = [0.001, 0.002]
    scales=[3, 3.5]
    resid_scale=2 
