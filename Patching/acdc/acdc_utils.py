...
def reset_network(task: str, device, model: torch.nn.Module) -> None:
    filename = {
        "IOI_test": "ioi_reset_heads_neurons.pt",
...
# Munging utils
# ----------------------------------
"""def is_corr_equal(corr1:TLACDCCorrespondence, corr2: TLACDCCorrespondence):
    count_no_edges1 = corr1.count_no_edges()
    count_no_edges2 = corr2.count_no_edges()
    all_edges2 = corr2.all_edges()


    #if not count_no_edges1 == count_no_edges2:
        #print(f"Edge count corr1: {count_no_edges1} \n Edge count corr2: {count_no_edges2}")
    #    return False
    # big_dict[(child_name, child_index, parent_name, parent_index)] = edge
    for child_name, rest1 in corr1.edges.items():
        for child_index, rest2 in rest1.items():
            for parent_name, rest3, in rest2.items():
                for parent_idx, edge in rest3.items():
                    if all_edges2.get((child_name, child_index, parent_name, parent_idx)) == None: #.present
                        print(f"Edge from {parent_name} {parent_idx} to {child_name}, {child_index} not present in corr2")
                        #return False
