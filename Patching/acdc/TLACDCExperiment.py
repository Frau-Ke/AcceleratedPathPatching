...
import torch
from Patching.acdc.acdc_graphics import show
...
from Patching.acdc.global_cache import GlobalCache
from Patching.acdc.acdc_graphics import log_metrics_to_wandb
import warnings
import wandb
from Patching.acdc.acdc_utils import extract_info, shuffle_tensor
from Patching.acdc.TLACDCEdge import (
    TorchIndex,
    Edge, 
    EdgeType,
...
import time
from Patching.acdc.acdc_utils import next_key
...
class TLACDCExperiment:
    ...
    def __init__(
        ...
        reduced_search_space:Optional[dict]=None,
        verbose: bool = False,
        ...
        # search space is whole model
        self.corr = TLACDCCorrespondence.setup_from_model(self.model, use_pos_embed=use_pos_embed, reduced_search_space=self.reduced_search_space)
       
        if early_exit: 
            return
            
        self.reverse_topologically_sort_corr()  # FLOPS: one forward pass on 10 random tokens
        ...
        self.setup_corrupted_cache()  # FLOPs: one forward pass on corrupted inputs
        ...
        self.metric = lambda x: metric(x)
        self.second_metric = second_metric
        self.update_cur_metric(recalc_metric=True, recalc_edges=True)  # one forward pass on clean inputs
    ...
    """def reduce_exisiting_model_with_threshold2(self, init_graph):

        # TODO: efficency metrics
        #self.exp.reduce_exisiting_model_with_threshold(init_graph, threshold)
        self.corr = copy.deepcopy(init_graph)#.deep_copy()
        
        
        if init_graph is None:
            raise Exception("Enter this function only if an existing graph is given")
        
        for child_name, rest1 in list(self.corr.edges.items()):
            is_this_node_used = False
            
            resid_post_name = self.model.blocks[-1].hook_resid_post.name
            if child_name in  ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed", resid_post_name]:
                print("include (inital)")
                is_this_node_used = True
            
            for child_index, rest2 in list(rest1.items()):
                
                for parent_name, rest3 in list(rest2.items()):
                        for parent_index, edge in list(rest3.items()):
                            self.print_edge(child_name, child_index, parent_name, parent_index, edge.effect_size)

                            if edge.edge_type == EdgeType.PLACEHOLDER:
                                print("include (Placeholder)")
                                is_this_node_used = True
                                continue # include by default
                            
                            if self.verbose:
                                print(f"Metric of edge between {child_name}, {child_index} and {parent_name}, {parent_index} is {edge.effect_size}")
                                
                            if edge.effect_size is not None:
                                if self.abs_value_threshold:
                                    result = abs(edge.effect_size)
                                else:
                                    result = edge.effect_size      
                                
                                if result < self.threshold:
                                    if self.verbose:
                                        print("... so removing connection")
                                    #self.corr.add_node(child_node)
                                    self.corr.remove_edge(child_name, child_index, parent_name, parent_index)
                                    print("not include (threshold)" )
                                else:
                                    print("include (threshold)")
                                    if self.verbose:
                                        print("... so keeping connection")
                                    is_this_node_used = True
                                    print("Edge in Graph is from ", parent_name, parent_index, "to ", child_name, child_index, "metric is", edge.effect_size )
    ...
    def step(self, early_stop=False, testing=False, show_img=False, save_img=False, patch_mlp=False):
        ...
        self.update_cur_metric(recalc_metric=True, recalc_edges=True) # FLOPS: one forward pass
        ...
        is_this_node_used = False
        resid_post_name = self.model.blocks[-1].hook_resid_post.name
        if self.current_node.name in ["blocks.0.hook_resid_pre", "hook_pos_embed", "hook_embed", resid_post_name]:
            is_this_node_used = True
            
        if not patch_mlp:
            # include mlps by default
            if "hook_mlp_out" in self.current_node.name or "hook_mlp_out" in self.current_node.name:
                is_this_node_used = True
        ...
        for sender_name in sender_names_list:
            ...
            for sender_index in sender_indices_list:
                ...
                if "hook_mlp_out" in sender_name or "hook_mlp_out" in sender_name:
                    if self.verbose:
                        print("Include MLP by default")
                    is_this_node_used = True
                    edge.present = True
                    continue
        ...
        if is_this_node_used and self.current_node.incoming_edge_type.value != EdgeType.PLACEHOLDER.value:
            fname = f"{self.experiment_folder}/ims/img_new_{self.step_idx}.png"
            if show_img or save_img:
                ...
                if show_img:
                    ...
                    graph_byte=graph.draw(format="svg", prog="dot")
                    display(SVG(graph_byte))
    ...
    def remove_unsignificant_MLP_connections(self):
        """We can get a lot of insignificant connections to MLPs, which makes the graph very noisy. 
        So we remove those connections, if they are to or from an MLP and below the threshold"""
            ...
            for child_index, rest2 in list(rest1.items()):
                for parent_name, rest3 in list(rest2.items()):
                        for parent_index, edge in list(rest3.items()):
                            ...
                            if "hook_mlp_out" in child_name or "hook_mlp_out" in parent_name:
                                if edge.effect_size is not None:
                                    if self.abs_value_threshold:
                                        result = abs(edge.effect_size)
                                    else:
                                ...
                                if  result < self.threshold:
                                    ...
                                    self.corr.remove_edge(child_name, child_index, parent_name, parent_index)
                                    if self.verbose:
                                        print("Removing insignificant MLP connection from ", parent_name, parent_index, "to ", child_name, child_index, "with effect size", edge.effect_size)                    
