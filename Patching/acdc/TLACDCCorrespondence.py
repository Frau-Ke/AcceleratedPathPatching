...
from Patching.acdc.acdc_utils import OrderedDefaultdict, make_nd_dict
...
class TLACDCCorrespondence:
    ...
    @classmethod
    def setup_from_model(cls, model, use_pos_embed=False, reduced_search_space=None):
        correspondence = cls()
        ...
        for layer_idx in range(model.cfg.n_layers - 1, -1, -1):
            ...
            if reduced_search_space is not None:
                reduced_heads_per_layer = reduced_search_space.get(layer_idx)

                if reduced_heads_per_layer == None or  len(reduced_heads_per_layer) == 0:
                    # dont add hooks to nodes that are not present in the reduced search space
                    continue
            ...
            for head_idx in range(model.cfg.n_heads - 1, -1, -1):
                if reduced_search_space is not None:
                    if head_idx not in reduced_heads_per_layer:
                        continue
