...
def ave_logits_diff_vanilla(    
    ...
    """ Standard logit difference: only a single correct and wrong answer for each sample. 
    Logit(correct_answer) - Logits(wrong answer)

    Returns:
        Float[Tensor, "batch"]: elemntwise logit difference
    """
...
def ave_logit_diff_dosctring(
    ...
    """Logit difference for docstring tasks has one correct answer, but mulitple different wrong answers. 
    Logit(correct_answer) - max(Logit(all_wrong_answeres))
...
def ave_logits_diff_greater_than(
    ...
    """Logit Difference for GreaterThan has multiple correct and wrong answers. All numbers YY larger than a number XX are 
    correct answers, all numbers YY smaller than XX are wrong answers. The mean over all correct and wrong answer is computed.
     
    - Models (e.g. GPT-2) encode XX and YY with a single token -> correct_answers = YY >= XX
    - Models (e.g Qwen), where XX and YY are encoded in two seperate tokens X1 X2 -> correct_answers = (Y1 > X1, all Y2) + (Y1 == X1, Y2 > X2)

    Raises:
        Exception: GreaterThan is not explicitly implemented for the model, check if numbers are tokenized like GPT-2 or Qwen

    Returns:
        Float[Tensor, "batch"]: elemntwise avg_logit_diff
    """
