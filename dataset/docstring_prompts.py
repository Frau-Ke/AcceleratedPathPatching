...
This task is implemented by https://github.com/jettjaniak/mi_utils_public/blob/main/prompts.py 
and modified by https://github.com/ArthurConmy/Automatic-Circuit-Discovery/blob/main/acdc/docstring/prompts.py#L18
"""
...
def filter_variables(variables:list[str], tokenizer):
    VARIABLES = []
    n_removed = 0
    for v in variables:
...
def filter_nouns(nouns:list[str], tokenizer):
    NOUNS = []
    n_removed = 0
    for n in nouns:
...
class BatchedPrompts:
    @typechecked
    def __init__(self, prompts: List[Prompt], tokenizer, prepend_bos=True, device="cuda"):
        self.clean_prompt = [(tokenizer.bos_token if prepend_bos else "") + p.clean_prompt for p in prompts]
        if isinstance(prompts[0].corrupt_prompt, str):
            self.corrupt_prompt = [(tokenizer.bos_token if prepend_bos else "") + p.corrupt_prompt for p in prompts]
        else:
            self.corrupt_prompt = {key: [(tokenizer.bos_token if prepend_bos else "") + p.corrupt_prompt[key] for p in prompts] for key in prompts[0].corrupt_prompt.keys()}
        ...
        self.clean_tokens = torch.stack([tokenizer(batch, padding=True, return_tensors ="pt").input_ids.long().to(device)[0] for batch in self.clean_prompt])
        
        if isinstance(prompts[0].corrupt_prompt, str):
            self.corrupt_tokens =  torch.stack(t.Tensor(tokenizer(batch, padding=True, return_tensors ="pt").input_ids.long().to(device)[0] for batch in self.corrupt_prompt))
        else:
            self.corrupt_tokens = {
                key: 
                torch.stack([tokenizer(batch, padding=True, return_tensors ="pt").input_ids.long().to(device)[0] for batch in prompts])
                for key, prompts in self.corrupt_prompt.items()
            }
            
        # [batch, n_correct_tokens]
        self.correct_tokens = torch.stack([tokenizer(batch, padding=True, return_tensors ="pt").input_ids.long().to(device)[:, 0] for batch in self.correct_answers])
        # [batch, n_wrong_tokens]
        self.wrong_tokens = torch.stack([tokenizer(batch, padding=True, return_tensors ="pt").input_ids.long().to(device)[:, 0] for batch in self.wrong_answers])
...
def docstring_induction_prompt_generator(
    ...
    *,
    ...
    random.seed(seed)
    variable_names_filterd = filter_variables(variable_names, tokenizer=tokenizer)
    common_single_token_nouns_filtered = filter_nouns(common_single_token_nouns, tokenizer=tokenizer)
    n_not_matching_args = n_matching_args - 1
    met_name, *all_args = random.sample(variable_names_filterd, 2+n_matching_args+n_not_matching_args+n_not_matching_args+n_matching_args+n_def_prefix_args+n_def_suffix_args+n_doc_prefix_args)
    args_ = list(all_args)
    ...
    met_desc_words = random.sample(common_single_token_nouns_filtered, met_desc_len)
    doc_args_desc_words = [random.sample(common_single_token_nouns_filtered, arg_desc_len) for _ in clean_doc_args]
