"""
This Implementation of the IOI dataset is taken from
https://github.com/callummcdougall/ARENA_3.0/blob/main/chapter1_transformer_interp/exercises/part3_indirect_object_identification/ioi_dataset.py
"""
...
def get_name_idxs(prompts, tokens, tokenizer, idx_types=["IO", "S1", "S2"], prepend_bos=False):
    name_idx_dict = dict((idx_type, []) for idx_type in idx_types)
    for prompt, toks in zip(prompts, tokens):
        
        IO_token = t.tensor(tokenizer(" " + prompt["IO"]).input_ids)
        IO_idx = t.nonzero((toks == IO_token))[0].item()
        name_idx_dict["IO"].append(IO_idx)
        
        S_token = t.tensor(tokenizer(" " + prompt["S"]).input_ids)
        S_idxs = t.nonzero((toks == S_token))
        name_idx_dict["S1"].append(S_idxs[0].item())
        
        try:
            name_idx_dict["S2"].append(S_idxs[1].item())
        except:    
                name_idx_dict["S1"].append(S_idxs[0].item())

            
    return [
        t.tensor(name_idx_dict[idx_type])
...
def get_end_idxs(toks, tokenizer, name_tok_len=1, prepend_bos=False,  model_name="gpt2", start_idx=None, device="cuda"):
    ...
    if "Qwen" in model_name:
        # since pad and eos token are different, QWEN does not neeed to use the relevant_idx = int(prepend_bos) trick!
        end_idxs = []
        for i in range(toks.shape[0]):
            end_idxs.append(end_of_prompt(toks[i], tokenizer=tokenizer, end_text="<|im_end|>\n", device="cuda"))
    else:
        end_idxs = t.full((toks.size(0),), fill_value=toks.size(1))
    return end_idxs


def get_target_idx(ioi_prompts, tokens, tokenizer, prepend_bos):
    target_idxs = []
    for prompt, toks in zip(ioi_prompts, tokens):
        #IO_token = tokenizer(" " + prompt["IO"]).input_ids[0]
        #target_idx = t.nonzero((toks == IO_token))[-1].item() - 1
        target_idx = len(tokenizer(prompt["text"]).input_ids)-2
        target_idxs.append(target_idx)
    return t.tensor(target_idxs) + int(prepend_bos)
    


def get_idx_dict(ioi_prompts, tokenizer, prepend_bos, toks, model_name="gpt2"):
    ...
    if "Qwen" in model_name:
        user_token = tokenizer.encode("user")[0]
        # the position where the prompt starts, directly after "user\n"
        start_idx = [start_of_prompt(tok, tokenizer=tokenizer, start_text="<|im_start|>user\n") for tok in toks]
        ...
        start_idx= t.zeros_like(end_idxs)
    ...
    return {
        ...
        "ends": end_idxs,
        "starts": start_idx,
        "punct": punct_idxs + start_idx[0],
        "target": target_idxs + start_idx[0]
    }
...
class IOIDataset:
    def __init__(
        ...
        if tokenizer is None:
            if ["gpt2", "Qwen"] in model_name:
                self.tokenizer = AutoTokenizer.from_pretrained(model_name)
                self.tokenizer.pad_token = self.tokenizer.eos_token
            else:
                raise Exception("A tokenizer must be give to create the dataset")
        else:
            self.tokenizer = tokenizer

        # remove names that are tokenized with more than one token
        if "Llama" in model_name:
            # Due to the space before the name, Llama tokenizer uses one extra tokens
            max_name_encoding = 3
        else:
            max_name_encoding = 1
        ...
        for i in range(len(NAMES)):
            name = NAMES[i]
            if len(self.tokenizer.encode(" " + name)) > max_name_encoding:
                long_names.append(name)
        NAMES = [name for name in NAMES if name not in long_names]
        ...
        #if "Qwen" in model_name:
        if False:
            print("Use chat template for IOI")
            # Qwen2.5 models use the Chat template. A user and assistance role is added. 
            # this influences the subject and object position
            formatted_prompts = [
            ...
            self.toks, self.attention_mask = tokenizer(formatted_prompts, padding=True, return_tensors="pt").values()
        else:
            self.toks, self.attention_mask = tokenizer(texts, padding=True,  return_tensors="pt").values()
        ...
        if remove_target_token:
            print("removing target token from toks")
            slices_start = [self.toks[i, : self.word_idx["target"][i]+1] for i in range(self.toks.size(0))]
            slices_end = [self.toks[i, self.word_idx["target"][i]+2  : ] for i in range(self.toks.size(0))]
            #toks = t.zeros((self.toks.size(0), self.toks.size(1)-int(prepend_bos))).int()
            toks = t.zeros((self.toks.size(0), self.toks.size(1)-1)).int()

            for i in range(self.toks.size(0)):
                s1= slices_start[i]
                s2 = slices_end[i]
                #print(tokenizer.decode(t.cat((s1, s2))))
                toks[i] = t.cat((s1, s2), dim=0)
            
            self.toks=toks
            self.word_idx["ends"] = t.tensor([end - 1 for end in self.word_idx["ends"]])
        ...
        if "Llama" in self.model_name:
            self.io_tokenIDs = [
                self.tokenizer.encode(" " + prompt["IO"])[-1] for prompt in self.ioi_prompts
            ]
            self.s_tokenIDs = [
                self.tokenizer.encode(" " + prompt["S"])[-1] for prompt in self.ioi_prompts
            ]
        else:
    ...
    def gen_flipped_prompts(self, flip, device):
        ...
        flipped_ioi_dataset = IOIDataset(
            ...
            remove_target_token=self.remove_target_token
        )
